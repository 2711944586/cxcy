...
本地工作区中可能保留 `数据以及图/` 和 `参考/` 两类原始素材目录。它们包含大体积原始表、图片和参考计划书，不参与静态站部署，也不纳入 Git 仓库提交；最终可交付版本以 `deliverables/` 和 `skyguard-plan/` 中的整理后材料为准。
...
生成脚本会刷新 `deliverables/` 下的 Word、PDF、图表、截图包索引、财务表和附录材料。若需要从原始数据重新生成，请先在本地恢复 `数据以及图/` 素材目录；重新生成前请关闭正在打开的 Word/WPS 文档窗口，避免文件被锁定。
